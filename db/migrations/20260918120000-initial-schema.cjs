...
/**
 * ============================================================================
 * Initial schema — expense-calculator
 * ============================================================================
 *
 * This is the app's real first migration, adapted at G3 scaffold time from
 * the G-Schema-approved draft at
 * projects/expense-calculator/decisions/migrations/00000000000001-initial-schema.js
 * (project.json G-Schema gate, decided CONFIRM by sales@webdesksolution.ca on
 * 2026-09-18T07:12:13Z). Content is unchanged from the approved draft other
 * than this header and the Sequelize-CLI timestamp-based filename — no new
 * structural decision was made converting it into a real migration.
 *
 * NOT YET RUN against any database in this environment: no local
 * Postgres/docker is available here (per the G3 task brief). This file is
 * structurally part of the app (correct location, correct Sequelize-CLI
 * naming convention, loadable by `sequelize-cli db:migrate`) but has not been
 * executed against a live database. That first real run is a G3/M1
 * follow-up once a dev-store-connected Postgres instance exists (OQ-1/OQ-3).
 *
 * Companion doc: projects/expense-calculator/decisions/data-model.md — every
 * constraint below is explained and tied back to a numbered constraint from
 * the architect's G-Schema handoff (architecture-packet.md §8) or to a
 * spec.md deliverable.
 *
 * Style note: this migration is written mostly as raw SQL via
 * `queryInterface.sequelize.query(...)`, rather than queryInterface's
 * table-builder DSL. That's a deliberate choice, not a shortcut: this schema
 * leans heavily on multi-column CHECK constraints, partial indexes, deferred
...
module.exports = {
  async up(queryInterface /*, Sequelize */) {
    await queryInterface.sequelize.transaction(async (transaction) => {
      ...
      // EXPENSE_CATEGORIES code constant (app/domain/expense-categories.ts,
      // data-model.md §2) — keep in sync.
      // ----------------------------------------------------------------------
      await q(`
